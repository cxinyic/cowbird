...
class FasterKv {
 ...
 public:
  ...
  void StopSession();
  void Refresh();
  void connect_rdma(char *server_ip, int num_threads);
...
// Implementations.
template <class K, class V, class D>
void FasterKv<K, V, D>::connect_rdma(char *server_ip, int num_threads) {
  memset(&disk.rdma_ctx, 0, sizeof (disk.rdma_ctx));
  disk.rdma_ctx.server = 0;
  disk.rdma_ctx.init_param(server_ip, num_threads);
  disk.rdma_ctx.init_connect();
  disk.rdma_ctx.send_buffer_mr();
}
template <class K, class V, class D>
...
inline void FasterKv<K, V, D>::CompleteIoPendingRequests(ExecutionContext& context) {
  ...
  // not yet completed.)
  int idx;
  // printf("begin CompleteIoPendingRequests\n");
  idx = Thread::id()%disk.rdma_ctx.nr_workers - 1;
  int i = 0;
  if (idx < 0) {
    idx += disk.rdma_ctx.nr_workers;
  }
  int nr_before_RW_process = disk.rdma_ctx.nr_read_response[idx] + disk.rdma_ctx.nr_write_response[idx];
  disk.rdma_ctx.poll_RW_response(idx);
  /*if (!context.io_responses.empty()){
    // printf("begin process_RW,  idx %d\n", idx);
    //printf("XY: rdma: idx %d, disk.rdma_ctx.c_cb_pool[idx].response_buffer->tail %d\n", idx, disk.rdma_ctx.c_cb_pool[idx].response_buffer->tail);
    //printf("XY: rdma: idx %d, disk.rdma_ctx.c_cb_pool[idx].response_buffer->head %d\n", idx, disk.rdma_ctx.c_cb_pool[idx].response_buffer->head);  
    while (disk.rdma_ctx.pre_local_read(idx, BATCH_SIZE)) {
      disk.rdma_ctx.poll_RW_response(idx);
    }
  
    
  } else {
    // printf("io response empty\n");
  }*/
  
  int nr_after_RW_process = disk.rdma_ctx.nr_read_response[idx] + disk.rdma_ctx.nr_write_response[idx];
  int nr_finish = nr_after_RW_process - nr_before_RW_process;
  
  // if (nr_finish > 0)
  //   printf("nr_finish is %d, idx %d\n", nr_finish, idx);
  ...
  while(context.io_responses.try_pop(ctxt)) {
    ...
    if(!pending_context.async) {
      // printf("XY: !pending_context.async\n");
...
inline Status FasterKv<K, V, D>::IssueAsyncIoRequest(ExecutionContext& ctx,
  ...
  // printf("XY: IssueAsyncIoRequest: id : %d\n", io_id);
  ...
  // printf("XY: AsyncGetFromDisk case0\n");
...
void FasterKv<K, V, D>::AsyncGetFromDisk(Address address, uint32_t num_records,
  ...
  ++num_pending_ios;
  // printf("XY: call AsyncGetFromDisk\n");
...
void FasterKv<K, V, D>::AsyncGetFromDiskCallback(IAsyncContext* ctxt, Status result,
  ...
  if(result == Status::Ok) {
    ...
    if(record->min_disk_key_size() > record_size) {
      ...
      printf("XY: AsyncGetFromDisk case2\n");
